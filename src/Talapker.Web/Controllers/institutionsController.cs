...
using Talapker.Application.InstitutionFeatures.Commands;
...
using Talapker.Application.InstitutionFeatures.Queries.GetAll;
...
public class InstitutionsController(IMessageBus messageBus) : ControllerBase
{
    ...
    [HttpDelete("{institutionId:guid}/admins/{userId:guid}")]
    public async Task<IActionResult> DeleteInstitutionAdminRole(
        Guid institutionId,
        Guid userId,
        [FromServices] UserManager<ApplicationUser> userManager)
    {
        var result = await messageBus.InvokeAsync<ApiResponse>(new DeleteInstitutionAdminRoleCommand(userId, institutionId));
